...
async function criarProduto(dados) {
  const { nome, descricao, modelo, marca, preco, imagem_url, ativo } = dados;
  if (!nome || !modelo || !preco || !marca) {
    throw new Error("Nome, modelo, preço,  e marca são obrigatórios"); // outro jeito de escrever "error", mas acho que é mais "PROFISSIONAL"
  }
  ...
  try {
    ...
    console.error("Erro ao criar estoque para o produto:", estoqueError);
  }
  return novoProduto;
}
...
async function atualizarProdutoCompleto(id, dados) {
  ...
  if (!nome || !modelo || !preco || !marca) {
    throw new Error("Nome, modelo, preço e marca são obrigatórios");
  }
