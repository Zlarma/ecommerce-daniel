  ...
  if (token && currentUser && currentUser.tipo_usuario === "ADMIN") {
    ...
    Promise.all([
      ...
      .then(([produtos, estoques]) => {
        ...
        resTabela.innerHTML = "";
        resTabela.innerHTML += `<table class="admin-table">${gerarTabela(
          produtos,
          estoques
        )}</table>`;
      })
    ...
    cadProd.addEventListener("click", (e) => {
      ...
      fetch(`http://localhost:3000/produto`, {
        ...
        .then((dados) => {
          ...
          resProd.innerHTML = "";
          resProd.innerHTML +=
            dados.mensagem || "Produto cadastrado com sucesso!";
...
function gerarTabela(produtos, estoques) {
  ...
  produtos.forEach((produto) => {
    ...
    const statusClass = produto.ativo
      ? "admin-status active"
